CCNP v2.0 R&S
=============================

Routing and Switching

.. toctree::
   :maxdepth: 1
   :caption: Contents:
   
   route
   switch
   tshoot

.. contents:: TOC

=========================================
試験前チートシート
=========================================

* :doc:`/cheat-sheets/cisco/ccnp-v2.0-rs/index`

==========================
試験一覧
==========================

* 300-101J ROUTE
* 300-115J SWITCH
* 300-135J TSHOOT

====================================================
私の試験順序（予定）
====================================================

#. 300-101J ROUTE
#. 300-115J SWITCH
#. 300-135J TSHOOT

====================================================
学習ポリシー
====================================================

#. 自分はどんくさくて効率が良くないし、頭が良くないということを頭に叩き込んでおく（つまり他の人より倍以上努力が必要）
#. 試験は 1 つずつ勉強していく。
#. お金もそんなにかけられないから本の購入も合格してから購入するスタンスで。
#. CCNP はそんな気軽に受けられる試験ではないので必ず Pearson VUE で予約してからいく。
#. 通院とセットでやるので、通院の都合が合わなければ都度リスケ
#. 最短を目指す。3 回受験で Pass するのが SHOULD 。

学習方法
---------------------

#. 教科書1週　実機演習込み
#. 問題集1週　実機演習込み
#. ping-t1週　実機演習込み
#. tut お試し　実機演習込み

プラン（リードオンリー）
-----------------------------------------

#. `CCNP_v2.0_R&S_学習計画 - Google スプレッドシート <https://docs.google.com/spreadsheets/d/1dhIfYZ9kJ1rb_aRtkbGTs4VZRo1KyTyYEBOqiGqrCnk/edit#gid=1427862754>`_

====================================================
学習ソース
====================================================

調査中、正直まだよくわかっていない。
あんまりお金かけられない。。。

教科書
-------------------

#. `Amazon.co.jp： 徹底攻略Cisco CCNP Routing & Switching ROUTE教科書［300-101J］対応 徹底攻略シリーズ eBook: 株式会社ソキウス・ジャパン: Kindleストア <https://www.amazon.co.jp/dp/B0179BCZG4>`_
#. `Amazon.co.jp： 徹底攻略 Cisco CCNP Routing & Switching SWITCH教科書［300-115J］対応 徹底攻略シリーズ eBook: 株式会社ソキウス・ジャパン: Kindleストア <https://www.amazon.co.jp/dp/B00ZU5WN8C>`_


CCNPイージス
---------------------

#. `CCNPイージス：CCNP SWITCH（300-115J）と ROUTE（300-101J）のWeb教科書 <http://www.infraexpert.com/info/ccnpz00.html>`_

Router Alley
---------------------

#. `Router Alley - Guides <http://www.routeralley.com/guides.html>`_

tut
--------------

+-------------------------------------------+----------------------+
| Site                                      | Track                |
+===========================================+======================+
| http://www.digitaltut.com                 | ROUTE                |
+-------------------------------------------+----------------------+
| http://www.certprepare.com                | SWITCH               |
+-------------------------------------------+----------------------+
| http://www.networktut.com                 | TSHOOT               |
+-------------------------------------------+----------------------+

支払い
^^^^^^^^^^^^^^^^^^

* 課金は各サイト毎
* 支払いは PayPal

1ヶ月9ドルで期限が切れると利用できなくなる。
支払いはペイパルのほか、今はビットコインも使えるらしい。

ping-t
--------------------

#. 問題集
#. コマ問

シミュレーション
-------------------------------

#. 実機

  * C1841 IOS 15系
  * C1812J IOS 12系
  * Catalyst 2960 IOS 15系
  * Catalyst 3750 IOS 15系

#. シミュレーター
#. ping-t のコマ問？

問題集
-------------------------------

書籍
^^^^^^^^^^^^^^

#. `徹底攻略Cisco CCNP Routing & Switching ROUTE問題集[300-101J]対応 | 株式会社ソキウス・ジャパン |本 | 通販 | Amazon <https://www.amazon.co.jp/dp/4844380001>`_
#. `徹底攻略 Cisco CCNP Routing & Switching SWITCH 問題集[300-115J]対応 | 株式会社ソキウス・ジャパン |本 | 通販 | Amazon <https://www.amazon.co.jp/dp/484433977X>`_
#. `徹底攻略 Cisco CCNP Routing & Switching TSHOOT 問題集[300-135J]対応 | 株式会社ソキウス・ジャパン |本 | 通販 | Amazon <https://www.amazon.co.jp/dp/4844380532>`_

Kindle
^^^^^^^^^^^^^^^^^^

#. `Amazon.co.jp： 徹底攻略Cisco CCNP Routing & Switching ROUTE問題集［300-101J］対応 徹底攻略シリーズ eBook: 株式会社ソキウス・ジャパン: Kindleストア <https://www.amazon.co.jp/dp/B01BU1H1U4>`_
#. `Amazon.co.jp： 徹底攻略Cisco CCNP Routing & Switching SWITCH問題集［300-115J］対応 徹底攻略シリーズ eBook: 株式会社ソキウス・ジャパン: Kindleストア <https://www.amazon.co.jp/dp/B0191B5FFS>`_
#. `Amazon.co.jp： 徹底攻略 Cisco CCNP Routing & Switching TSHOOT 問題集［300-135J］対応 徹底攻略シリーズ eBook: 株式会社ソキウス・ジャパン: Kindleストア <https://www.amazon.co.jp/dp/B01EHHJXV8>`_

==========================
情報収集
==========================

#. `CCNP Routing and Switching - Cisco <http://www.cisco.com/c/ja_jp/training-events/training-certifications/certifications/professional/ccnp-routing-switching.html>`_
#. `CCNPイージス：CCNP SWITCH（300-115J）と ROUTE（300-101J）のWeb教科書 <http://www.infraexpert.com/info/ccnpz00.html>`_
#. `一番効率的なCCNP勉強方法。CCIE取得後に考えました。 <http://ccie-go.com/ccnp-study-1/>`_
#. `【ワカヤマン】CCNP総合15 [無断転載禁止]©2ch.net <http://hanabi.2ch.net/test/read.cgi/lic/1476606631/>`_
#. `CCNA/CCNP/LPIC/オラクルマスター/ITパスポート/ITILファンデーション総合学習サイト <https://ping-t.com/>`_
#. `Cisco - pg1x wiki <https://wiki.pg1x.io/index.php?title=Cisco>`_
